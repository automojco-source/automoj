...
export function HomeView() {
  ...
  useEffect(() => {
    ...
    // Anyone who has asked their system for less motion gets the poster frame
    // and nothing else — no scroll-driven sequence, no 61 extra requests.
    ...
    const loadFrame = (index: number) => {
      ...
      img.decoding = "async";
      img.src = `/frames/f_${String(index + 1).padStart(3, "0")}.webp`;
      if (index === 0) {
        img.onload = () => {
    ...
    // The first frames are the ones a visitor sees while still at the top of
    // the page. Firing all 61 requests at mount made them compete with the
    // page's own critical assets for no benefit, so the rest wait for idle time.
    const EAGER_FRAMES = 10;
    ...
    if (!prefersReducedMotion) {
      for (let i = 1; i < Math.min(EAGER_FRAMES, TOTAL_FRAMES); i++) loadFrame(i);
      ...
      if (typeof window.requestIdleCallback === "function") {
        idleHandle = window.requestIdleCallback(loadRemaining, { timeout: 3000 });
      } else {
        idleIsTimeout = true;
        idleHandle = window.setTimeout(loadRemaining, 1200);
      }
    }
  ...
  // Under reduced motion every line is simply present: the hero reads as a
  // finished still rather than something waiting to be scrolled into existence.
  const subLine1 = reducedMotion ? 1 : Math.min(1, scrollProgress * 5);
  const subLine2 = reducedMotion ? 1 : getSubProgress(scrollProgress, 0.15, 0.48);
  const subLine3 = reducedMotion ? 1 : getSubProgress(scrollProgress, 0.45, 0.78);
  const subDesc = reducedMotion ? 1 : getSubProgress(scrollProgress, 0.70, 0.95);
  const scrollIndicatorOpacity = reducedMotion ? 0 : Math.max(0, 1 - scrollProgress * 4.5);
    ...
    <main className="min-h-screen bg-[#080808] text-[#EDEDED] font-sans">
      ...
      >
        ...
        <div className="sticky top-14 sm:top-16 h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-4rem)] w-full overflow-hidden flex items-start justify-start">
          ...
          >
            {/* The canvas is empty until JavaScript has run, which meant a black
                screen for anyone on a slow connection and no hero image at all
                for a crawler. This is the same first frame, served as a real
                image and marked priority so it is the LCP candidate; the canvas
                paints over it once the sequence takes over. */}
            <NextImage
              src="/frames/f_001.webp"
              alt="Panel work underway in the Auto Moj workshop"
              fill
              priority
              sizes="100vw"
        ...
        <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="scroll-reveal scroll-delay-1 bg-[#0C0C0C] border border-[#1C1C1C] p-4 flex flex-col group hover:border-[#C5A880]/50 transition-all duration-300">
              <div className="relative aspect-[4/3] overflow-hidden bg-black mb-4 border border-[#171717]">
                <NextImage
                  src="/images/door_restoration.jpg"
                  alt="Scratch and dent repair at Auto Moj"
                  fill
            ...
            <div className="scroll-reveal scroll-delay-2 bg-[#0C0C0C] border border-[#1C1C1C] p-4 flex flex-col group hover:border-[#C5A880]/50 transition-all duration-300">
              <div className="relative aspect-[4/3] overflow-hidden bg-black mb-4 border border-[#171717]">
                <NextImage
                  ...
                  alt="Respray and refinishing at Auto Moj"
                  fill
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                  className="object-cover transition-transform duration-700 group-hover:scale-105 filter brightness-95"
                />
            ...
            <div className="scroll-reveal scroll-delay-3 bg-[#0C0C0C] border border-[#1C1C1C] p-4 flex flex-col group hover:border-[#C5A880]/50 transition-all duration-300">
              <div className="relative aspect-[4/3] overflow-hidden bg-black mb-4 border border-[#171717]">
                <NextImage
                  ...
                  alt="Paintless dent removal at Auto Moj"
                  fill
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                  className="object-cover transition-transform duration-700 group-hover:scale-105 filter brightness-95"
                />
            ...
            <div className="scroll-reveal scroll-delay-4 bg-[#0C0C0C] border border-[#1C1C1C] p-4 flex flex-col group hover:border-[#C5A880]/50 transition-all duration-300">
              <div className="relative aspect-[4/3] overflow-hidden bg-black mb-4 border border-[#171717]">
                <NextImage
                  ...
                  alt="Traditional panel beating at Auto Moj"
                  fill
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw"
                  className="object-cover transition-transform duration-700 group-hover:scale-105 filter brightness-95"
                />
              ...
              <div className="mt-auto pt-3 text-center border-t border-[#171717]">
                <Link href="/services/panel-beating" className="inline-flex items-center gap-1 text-[9px] tracking-[0.25em] uppercase text-[#777777] group-hover:text-[#C5A880] transition-colors">
            ...
            <div className="lg:col-span-7 flex justify-center lg:justify-end scroll-reveal-right">
              <div className="w-full max-w-lg border border-[#1C1C1C] bg-[#0C0C0C] p-3 sm:p-4">
                <NextImage
                  ...
                  alt="Metal being shaped by hand at Auto Moj"
                  width={1024}
                  height={1024}
            ...
            <div className="lg:col-span-6 scroll-reveal-left">
              <div className="border border-[#1C1C1C] bg-[#0C0C0C] p-3 sm:p-4">
                <NextImage
                  ...
                  alt="Composite bumper repair at Auto Moj"
                  width={1024}
                  height={640}
                  sizes="(max-width: 1024px) 100vw, 50vw"
